...
func on_choice_selected(choice_index: int):
	...
	if choice_index == 0:
		$VideoStreamPlayerL.show()
		$VideoStreamPlayerL.play()
	else:
		$VideoStreamPlayerR.show()
		$VideoStreamPlayerR.play()
	...
	if choice.has("next_scene"):
		fade_out_and_load_scene(choice.next_scene, 3)
